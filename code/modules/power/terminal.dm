...
/obj/machinery/power/terminal/proc/dismantle(mob/living/user, obj/item/I)
	if(isturf(loc))
		var/turf/T = loc
		if(T.intact)
			to_chat(user, span_warning("I must first expose the power terminal!"))
			return
	...
	user.visible_message("[user.name] dismantles the power terminal from [master].",
		span_notice("I begin to cut the cables..."))
	...
	if(I.use_tool(src, user, 50))
		...
		to_chat(user, span_notice("I cut the cables and dismantle the power terminal."))
		qdel(src)
