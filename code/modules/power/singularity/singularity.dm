...
/obj/singularity/attack_tk(mob/user)
	if(iscarbon(user))
		...
		C.visible_message(span_danger("[C]'s head begins to collapse in on itself!"), span_userdanger("My head feels like it's collapsing in on itself! This was really not a good idea!"), span_italic("I hear something crack and explode in gore."))
		for(var/i in 1 to 3)
...
/obj/singularity/proc/combust_mobs()
	for(var/mob/living/carbon/C in urange(20, src, 1))
		C.visible_message(span_warning("[C]'s skin bursts into flame!"), \
						  span_userdanger("I feel an inner fire as your skin bursts into flames!"))
			...
			if (ishuman(M))
				...
				if(istype(H.glasses, /obj/item/clothing/glasses/meson))
					...
					if(MS.vision_flags == SEE_TURFS)
						to_chat(H, span_notice("I look directly into the [src.name], good thing you had your protective eyewear on!"))
						return

		M.apply_effect(60, EFFECT_STUN)
		M.visible_message(span_danger("[M] stares blankly at the [src.name]!"), \
						span_userdanger("I look directly into the [src.name] and feel weak."))
	return
