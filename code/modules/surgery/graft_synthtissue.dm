...
/datum/surgery_step/graft_synthtissue/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	user.visible_message("[user] successfully grafts synthtissue to [chosen_organ].", span_notice("I succeed in grafting 10u of the synthflesh to the [chosen_organ]."))
...
/datum/surgery_step/graft_synthtissue/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	user.visible_message(span_warning("[user] accidentally damages part of [chosen_organ]!"), span_warning("I damage [chosen_organ]! Apply more synthtissue if it's run out."))
