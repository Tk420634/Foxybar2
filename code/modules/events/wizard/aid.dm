		...
		if(L.mind && L.mind.spell_list.len != 0)
			...
			for(var/obj/effect/proc_holder/spell/S in L.mind.spell_list)
				if(S.clothes_req)
					...
					spell_improved = TRUE
			if(spell_improved)
				to_chat(L, span_notice("I suddenly feel like you never needed those garish robes in the first place..."))
...
/datum/round_event/wizard/improvedcasting/start()
	for(var/i in GLOB.mob_living_list)
		...
		if(L.mind && L.mind.spell_list.len != 0)
			...
			to_chat(L, span_notice("I suddenly feel more competent with your casting!"))
