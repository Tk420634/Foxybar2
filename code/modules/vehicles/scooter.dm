...
/obj/vehicle/ridden/scooter/buckle_mob(mob/living/M, force = 0, check_loc = 1)
	...
	if(M.get_num_legs() < 2 && M.get_num_arms() <= 0)
		to_chat(M, span_warning("My limbless body can't ride \the [src]."))
		return 0
	. = ..()
...
/obj/vehicle/ridden/scooter/wheelys/post_unbuckle_mob(mob/living/M)
	if(!has_buckled_mobs())
		to_chat(M, span_notice("I pop the Wheely-Heel's wheels back into place."))
		moveToNullspace()
	return ..()

/obj/vehicle/ridden/scooter/wheelys/post_buckle_mob(mob/living/M)
	to_chat(M, span_notice("I pop out the Wheely-Heel's wheels."))
	return ..()
