...
/obj/structure/disposalconstruct/proc/can_be_rotated(mob/user,rotation_type)
	if(anchored)
		to_chat(user, span_warning("I must unfasten the pipe before rotating it!"))
		return FALSE
	return TRUE
...
/obj/structure/disposalconstruct/wrench_act(mob/living/user, obj/item/I)
	if(anchored)
		anchored = FALSE
		density = FALSE
		to_chat(user, span_notice("I detach the [pipename] from the underfloor."))
	else
		var/ispipe = is_pipe() // Indicates if we should change the level of this pipe

		var/turf/T = get_turf(src)
		if(T.intact && isfloorturf(T))
			to_chat(user, span_warning("I can only attach the [pipename] if the floor plating is removed!"))
			return TRUE

		if(!ispipe && iswallturf(T))
			to_chat(user, span_warning("I can't build [pipename]s on walls, only disposal pipes!"))
			return TRUE
		...
		to_chat(user, span_notice("I attach the [pipename] to the underfloor."))
...
/obj/structure/disposalconstruct/welder_act(mob/living/user, obj/item/I)
	if(anchored)
		...
		to_chat(user, span_notice("I start welding the [pipename] in place..."))
		...
		to_chat(user, span_warning("I need to attach it to the plating first!"))
	return TRUE
