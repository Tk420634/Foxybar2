...
/mob/living/simple_animal/pet/dog/corgi/attackby(obj/item/O, mob/user, params)
	if (istype(O, /obj/item/razor))
		...
		if (nofur)
			...
			return
		user.visible_message("[user] starts to shave [src] using \the [O].", span_notice("I start to shave [src] using \the [O]..."))
...
/mob/living/simple_animal/pet/dog/corgi/proc/place_on_head(obj/item/item_to_add, mob/user)
	...
	if(inventory_head)
		...
		return
	if(!item_to_add)
		user.visible_message("[user] pets [src].",span_notice("I rest your hand on [src]'s head for a moment."))
	...
	if(valid)
		if(health <= 0)
			...
			user.visible_message("[user] puts [item_to_add] on [real_name]'s head.  [src] looks at [user] and barks once.",
				...
				span_italic("I hear a friendly-sounding bark."))
		item_to_add.forceMove(src)
