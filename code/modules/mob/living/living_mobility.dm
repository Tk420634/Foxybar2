...
/mob/living/proc/lay_down()
	...
	if(client?.prefs?.autostand)
		...
		to_chat(src, span_notice("I am now attempting to [(combat_flags & COMBAT_FLAG_INTENTIONALLY_RESTING) ? "[!resting ? "lay down and ": ""]stay down" : "[resting ? "get up and ": ""]stay up"]."))
		if((combat_flags & COMBAT_FLAG_INTENTIONALLY_RESTING) && !resting)
			if(istype(src.get_item_by_slot(SLOT_WEAR_SUIT),/obj/item/clothing/suit/armor/power_armor))
				to_chat(src, span_notice("I cannot lie down in that heavy armor!"))
				return
		...
		if(!resting)
			set_resting(TRUE, FALSE)
			to_chat(src, span_notice("I am now laying down."))
		else
			resist_a_rest()
