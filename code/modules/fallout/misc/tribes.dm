...
/datum/gang/tribe/add_leader(mob/living/carbon/new_leader)
	...
	to_chat(new_leader, span_notice("I have become a new leader of the [name]! You can now invite and remove members at will."))
...
/datum/gang/tribe/add_member(mob/living/carbon/new_member)
	...
	to_chat(new_member, span_notice("I am now a member of the [name]! Everyone can recognize your tribe."))
	if(welcome_text)
...
/mob/living/proc/leave_tribe()
	...
	var/datum/gang/tribe/G = gang
	if(!G)
		to_chat(src, "I am already not in any tribe!")
		return
...
/mob/living/proc/transfer_tribe_leader()
	...
	if(G && G.leader == src)
		...
		to_chat(src, span_notice("I have transferred tribe leadership of the [G.name] to [H.real_name]!"))
		to_chat(H, span_notice("I have received tribe leadership of the [G.name] from [src.real_name]!"))
		G.remove_leader(src)
		G.add_leader(H)
...
/mob/living/proc/set_tribe_welcome()
	...
	to_chat(src, span_notice("I have set a welcome text for a new tribe members!"))
...
/mob/living/proc/set_tribe_color()
	...
	to_chat(src, span_notice("I have chosen a new tribe color!"))
