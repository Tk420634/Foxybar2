...
/obj/item/integrated_circuit/input/button/ask_for_input(mob/user) //Bit misleading name for this specific use.
	to_chat(user, span_notice("I press the button labeled '[displayed_name]'."))
	activate_pin(1)
...
/obj/item/integrated_circuit/input/toggle_button/ask_for_input(mob/user) // Ditto.
	...
	push_data()
	activate_pin(1)
	to_chat(user, span_notice("I toggle the button labeled '[displayed_name]' [get_pin_data(IC_OUTPUT, 1) ? "on" : "off"]."))
...
/obj/item/integrated_circuit/input/sensor/sense(atom/A, mob/user, prox)
	...
	set_pin_data(IC_OUTPUT, 1, WEAKREF(A))
	push_data()
	to_chat(user, span_notice("I scan [A] with [assembly]."))
	activate_pin(1)
	return TRUE
...
/obj/item/integrated_circuit/input/obj_scanner/attackby_react(atom/A,mob/user,intent)
	...
	set_pin_data(IC_OUTPUT, 1, WEAKREF(A))
	push_data()
	to_chat(user, span_notice("I let [assembly] scan [A]."))
	activate_pin(1)
	return TRUE
