...
/obj/item/organ/genital/proc/climaxable(mob/living/carbon/human/H, silent = FALSE) //returns the fluid source (ergo reagents holder) if found.
	...
	if(!. && !silent)
		to_chat(H, span_warning("My [name] is unable to produce it's own fluids, it's missing the organs for it."))
...
/mob/living/carbon/human/proc/mob_climax_outside(obj/item/organ/genital/G, mb_time = 30) //This is used for forced orgasms and other hands-free climaxes
	...
	//if(!fluid_source)
		//to_chat(src,span_userdanger("My [G.name] cannot cum."))
		//return
	if(mb_time) //as long as it's not instant, give a warning
		to_chat(src,span_userlove("I feel yourself about to orgasm."))
		if(!do_after(src, mb_time, target = src) || !G.climaxable(src, TRUE))
			return
	to_chat(src,span_userlove("I climax[isturf(loc) ? " onto [loc]" : ""] with your [G.name]."))
...
/mob/living/carbon/human/proc/mob_climax_partner(obj/item/organ/genital/G, mob/living/L, spillage = TRUE, mb_time = 30) //Used for climaxing with any living thing
	...
	if(mb_time) //Skip warning if this is an instant climax.
		...
		if(!do_after(src, mb_time, target = src) || !in_range(src, L) || !G.climaxable(src, TRUE))
			return
	if(spillage)
		to_chat(src,span_userlove("I orgasm with [L], spilling out of them, using your [G.name]."))
		to_chat(L,span_userlove("[src] climaxes with you, overflowing and spilling, using [p_their()] [G.name]!"))
	else //knots and other non-spilling orgasms
		to_chat(src,span_userlove("I climax with [L], your [G.name] spilling nothing."))
...
/mob/living/carbon/human/proc/mob_fill_container(obj/item/organ/genital/G, obj/item/reagent_containers/container, mb_time = 30) //For beaker-filling, beware the bartender
	...
	if(!fluid_source)
		return
	if(mb_time)
		to_chat(src,span_userlove("I start to [G.masturbation_verb] your [G.name] over [container]."))
		if(!do_after(src, mb_time, target = src) || !in_range(src, container) || !G.climaxable(src, TRUE))
			return
	to_chat(src,span_userlove("I used your [G.name] to fill [container]."))
...
/mob/living/carbon/human/proc/pick_climax_genitals(silent = FALSE)
	...
	if(LAZYLEN(genitals_list))
		...
		to_chat(src, span_warning("I cannot climax without available genitals."))
...
/mob/living/carbon/human/proc/pick_partner(silent = FALSE)
	...
	if(!partners.len)
		if(!silent)
			to_chat(src, span_warning("I cannot do this alone."))
		return //No one left.
...
/mob/living/carbon/human/proc/pick_climax_container(silent = FALSE)
	...
	if(containers_list.len)
		...
		to_chat(src, span_warning("I cannot do this without an appropriate container."))
...
/mob/living/carbon/human/proc/mob_climax(forced_climax=FALSE) //Forced is instead of the other proc, makes you cum if you have the tools for it, ignoring restraints
	if(mb_cd_timer > world.time)
		if(!forced_climax) //Don't spam the message to the victim if forced to come too fast
			to_chat(src, span_warning("I need to wait [DisplayTimeText((mb_cd_timer - world.time), TRUE)] before you can do that again!"))
		return
	...
	if(stat == DEAD)
		if(!forced_climax)
			to_chat(src, span_warning("I can't do that while dead!"))
		return
	...
	if(stat == UNCONSCIOUS) //No sleep-masturbation, you're unconscious.
		to_chat(src, span_warning("I must be conscious to do that!"))
		return
