...
/obj/machinery/computer/emergency_shuttle/proc/attempt_hijack_stage(mob/living/user)
	...
	if(hijack_last_stage_increase >= world.time + hijack_stage_cooldown)
		...
		return
	hijack_hacking = TRUE
	to_chat(user, span_boldwarning("I [SSshuttle.emergency.hijack_status == NOT_BEGUN? "begin" : "continue"] to override [src]'s navigational protocols."))
	...
	if(do_after(user, hijack_stage_time * (1 / user.mind.get_hijack_speed()), target = src))
		...
		. = TRUE
		to_chat(user, span_notice("I reprogram some of [src]'s programming, putting it on timeout for [hijack_stage_cooldown/10] seconds."))
...
/obj/docking_port/mobile/emergency/cancel(area/signalOrigin)
	...
	if(prob(70))
		...
		SSshuttle.emergencyLastCallLoc = null
	priority_announce("The train has been recalled.[SSshuttle.emergencyLastCallLoc ? " Recall signal traced. Results can be viewed on any communications console." : "" ]", null, 'sound/roundend/get_fucked.ogg', "Vault-Tec")
	...
	switch(mode)
		...
		if(SHUTTLE_CALL)
			if(time_left <= 0)
				...
				priority_announce("The [SSshuttle.train_name] from [SSshuttle.where_from] has arrived at [SSshuttle.local_train_station]. It will depart for [SSshuttle.where_to] in [timeLeft(600)] minutes. Have your tickets ready.", null, 'sound/roundend/get_fucked.ogg', "[SSshuttle.train_company]")
				ShuttleDBStuff()
