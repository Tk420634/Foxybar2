...
/obj/item/assembly/health/AltClick(mob/living/user)
	if(alarm_health == HEALTH_THRESHOLD_CRIT)
		...
		to_chat(user, span_notice("I toggle [src] to \"detect death\" mode."))
	else
		alarm_health = HEALTH_THRESHOLD_CRIT
		to_chat(user, span_notice("I toggle [src] to \"detect critical state\" mode."))
...
/obj/item/assembly/health/attack_self(mob/user)
	. = ..()
	to_chat(user, span_notice("I toggle [src] [src.scanning ? "off" : "on"]."))
	toggle_scan()
