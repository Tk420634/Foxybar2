...
/obj/item/dronespeak_manual/attack_self(mob/living/user)
	...
	if(isdrone(user) || issilicon(user))
		if(user.has_language(/datum/language/drone))
			to_chat(user, span_boldannounce("I start skimming through [src], but you already know dronespeak."))
		else
			to_chat(user, span_boldannounce("I start skimming through [src], and suddenly the drone chittering makes sense."))
	...
	if(user.has_language(/datum/language/drone))
		...
		to_chat(user, span_boldannounce("I start skimming through [src], but you can't make any sense of the contents."))
...
/obj/item/dronespeak_manual/attack(mob/living/M, mob/living/user)
	...
	if(isdrone(M) || issilicon(M))
		if(M.has_language(/datum/language/drone))
			M.visible_message(span_danger("[user] beats [M] over the head with [src]!"), span_userdanger("[user] beats you over the head with [src]!"), span_hear("I hear smacking."))
		else
			M.visible_message(span_notice("[user] teaches [M] by beating [M.p_them()] over the head with [src]!"), span_boldnotice("As [user] hits you with [src], chitters resonate in your mind."), span_hear("I hear smacking."))
