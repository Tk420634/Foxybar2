			...
			if(bomb.timing)
				if(!alert)
					...
					if(isliving(loc))
						...
						to_chat(L, span_userdanger("My [name] vibrates and lets out a tinny alarm. Uh oh."))
...
/obj/item/pinpointer/nuke/proc/switch_mode_to(new_mode)
	if(isliving(loc))
		var/mob/living/L = loc
		to_chat(L, span_userdanger("My [name] beeps as it reconfigures it's tracking algorithms."))
