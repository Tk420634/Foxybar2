...
/datum/action/bloodsucker/fortitude/ActivatePower()
	...
	to_chat(user, span_notice("My flesh, skin, and muscles become as steel."))
	// Traits & Effects
	...
	while(B && ContinueActive(user) || user.m_intent == MOVE_INTENT_RUN)
		if(istype(user.buckled, /obj/vehicle)) //We dont want people using fortitude being able to use vehicles
			...
			if(VRD)
				...
				to_chat(user, span_notice("I trip off the [V], your muscles too heavy for it to support you."))
			else
				V.unbuckle_mob(user, force = TRUE)
				to_chat(user, span_notice("I fall off the [V], your weight making you too heavy to be supported by it."))
