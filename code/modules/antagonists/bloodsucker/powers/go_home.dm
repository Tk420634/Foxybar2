...
/datum/action/bloodsucker/gohome/CheckCanUse(display_error)
	...
	if(!istype(bloodsuckerdatum) || !bloodsuckerdatum.coffin)
		if(display_error)
			to_chat(owner, span_warning("My coffin has been destroyed!"))
		return FALSE
	return TRUE
...
/datum/action/bloodsucker/gohome/ActivatePower()
	...
	to_chat(user, span_notice("I focus on separating your consciousness from your physical form..."))
	...
	if(!istype(bloodsuckerdatum) || !bloodsuckerdatum.coffin)
		to_chat(user, span_warning("My coffin has been destroyed! You no longer have a destination."))
		return FALSE
	if(!owner)
		return
