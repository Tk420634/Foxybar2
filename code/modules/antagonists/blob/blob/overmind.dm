...
/mob/camera/blob/proc/set_strain(datum/blobstrain/new_strain)
	if (ispath(new_strain))
		...
		blobstrain.on_gain()
		if (hadstrain)
			to_chat(src, "My strain is now: <b><font color=\"[blobstrain.color]\">[blobstrain.name]</b></font>!")
...
/mob/camera/blob/Login()
	..()
	to_chat(src, span_notice("I am the overmind!"))
	blob_help()
	update_health_hud()
	add_points(0)
...
/mob/camera/blob/Stat()
	...
	if(statpanel("Status"))
		...
		if(free_strain_rerolls)
			stat(null, "I have [free_strain_rerolls] Free Strain Reroll\s Remaining")
		if(!placed)
