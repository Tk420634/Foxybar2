...
/obj/item/electroadaptive_pseudocircuit/proc/adapt_circuit(mob/living/silicon/robot/R, circuit_cost = 0)
	if(QDELETED(R) || !istype(R))
		return
	if(!R.cell)
		to_chat(R, span_warning("I need a power cell installed for that."))
		return
	if(!R.cell.use(circuit_cost))
		to_chat(R, span_warning("I don't have the energy for that (you need [DisplayEnergy(circuit_cost)].)"))
		return
	if(recharging)
		to_chat(R, span_warning("[src] needs some time to recharge first."))
		return
	if(!circuits)
		to_chat(R, span_warning("I need more material. Use [src] on existing simple circuits to break them down."))
		return
...
/obj/item/electroadaptive_pseudocircuit/afterattack(atom/target, mob/living/user, proximity)
	...
	span_notice("I recycle [target] into [src]. It now has material for <b>[circuits]</b> circuits."))
