	...
	if(iscarbon(user))
		...
		if(C.get_item_by_slot(SLOT_HEAD) == src)
			if(HAS_TRAIT_FROM(src, TRAIT_NODROP, REVERSE_BEAR_TRAP_TRAIT) && !struggling)
				...
				span_danger("I [fear_string] try to pull at [src]..."), "<i>You hear clicking and ticking.</i>")
				...
				if(!prob(escape_chance))
					...
					span_userdanger("I force open the padlock!"), "<i>You hear a single, pronounced click!</i>")
