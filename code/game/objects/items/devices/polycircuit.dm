...
/obj/item/stack/circuit_stack/attack_self(mob/user)// Prevents the crafting menu, and tells you how to use it.
	to_chat(user, span_warning("I can't use [src] by itself, you'll have to try and remove one of these circuits by hand... carefully."))
...
/obj/item/stack/circuit_stack/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
	...
	if(!user.get_inactive_held_item() == src)
		...
		to_chat(user, span_notice("I spot your circuit, and carefully attempt to remove it from [src], hold still!"))
		if(do_after(user, 30, target = user))
			...
			use(1)
			if(!amount)
				to_chat(user, span_notice("I navigate the sharp edges of circuitry and remove the last board."))
			else
				to_chat(user, span_notice("I navigate the sharp edges of circuitry and remove a single board from [src]"))
		else
			H.apply_damage(15, BRUTE, pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM))
			to_chat(user, span_warning("I give yourself a wicked cut on [src]'s many sharp corners and edges!"))
		..()
