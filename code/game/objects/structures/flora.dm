...
/obj/structure/flora/tree/attackby(obj/item/W, mob/user, params)
	if(log_amount && (!(flags_1 & NODECONSTRUCT_1)))
		if(W.sharpness && W.force > 0)
			...
			user.visible_message(span_notice("[user] begins to cut down [src] with [W]."),span_notice("I begin to cut down [src] with [W]."), "I hear the sound of sawing.")
			if(do_after(user, 1000/W.force, target = src)) //5 seconds with 20 force, 8 seconds with a hatchet, 20 seconds with a shard.
				user.visible_message(span_notice("[user] fells [src] with the [W]."),span_notice("I fell [src] with the [W]."), "I hear the sound of a tree falling.")
...
/obj/structure/flora/rock/attackby(obj/item/W, mob/user, params)
	...
	if(flags_1 & NODECONSTRUCT_1)
		return ..()
	to_chat(user, span_notice("I start mining..."))
	if(W.use_tool(src, user, 40, volume=50))
		to_chat(user, span_notice("I finish mining the rock."))
