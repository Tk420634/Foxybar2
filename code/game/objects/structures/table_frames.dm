	...
	if (istype(I, /obj/item/stack))
		if(material?.tableVariant)
			if(material.get_amount() < 1)
				to_chat(user, span_warning("I need one [material.name] sheet to do this!"))
				return
			to_chat(user, span_notice("I start adding [material] to [src]..."))
			if(do_after(user, 20, target = src) && material.use(1))
				make_new_table(material.tableVariant)
		else
			if(material.get_amount() < 1)
				to_chat(user, span_warning("I need one sheet to do this!"))
				return
			to_chat(user, span_notice("I start adding [material] to [src]..."))
			if(do_after(user, 20, target = src) && material.use(1))
