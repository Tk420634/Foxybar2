...
/obj/machinery/sparker/attackby(obj/item/W, mob/user, params)
	if (istype(W, /obj/item/screwdriver))
		...
		if (src.disable)
			user.visible_message("[user] has disabled \the [src]!", span_notice("I disable the connection to \the [src]."))
			icon_state = "[base_state]-d"
		if (!src.disable)
			user.visible_message("[user] has reconnected \the [src]!", span_notice("I fix the connection to \the [src]."))
			if(src.powered())
				icon_state = "[base_state]"
			else
				icon_state = "[base_state]-p"
