...
/obj/structure/camera_assembly/attackby(obj/item/W, mob/living/user, params)
	switch(state)
		if(1)
			...
			if(istype(W, /obj/item/weldingtool))
				if(weld(W, user))
					to_chat(user, span_notice("I weld the assembly securely into place."))
					setAnchored(TRUE)
					state = 2
				return
		if(2)
			...
			if(istype(W, /obj/item/stack/cable_coil))
				if(W.use_tool(src, user, 0, 2))
					to_chat(user, span_notice("I add wires to the assembly."))
					state = 3
				else
					to_chat(user, span_warning("I need two lengths of cable to wire a camera!"))
					return
				return
				...
				if(weld(W, user))
					to_chat(user, span_notice("I unweld the assembly from its place."))
					state = 1
					setAnchored(TRUE)
				return
	...
	if(is_type_in_typecache(W, possible_upgrades) && !is_type_in_list(W, upgrades)) // Is a possible upgrade and isn't in the camera already.
		if(!user.transferItemToLoc(W, src))
			return
		to_chat(user, span_notice("I attach \the [W] into the assembly inner circuits."))
		upgrades += W
		return
...
/obj/structure/camera_assembly/crowbar_act(mob/user, obj/item/tool)
	...
	if(U)
		to_chat(user, span_notice("I detach an upgrade from the assembly."))
...
/obj/structure/camera_assembly/wirecutter_act(mob/user, obj/item/I)
	...
	to_chat(user, span_notice("I cut the wires from the circuits."))
	state = 2
	return TRUE
...
/obj/structure/camera_assembly/wrench_act(mob/user, obj/item/I)
	if(state != 1)
		return FALSE
	I.play_tool_sound(src)
	to_chat(user, span_notice("I detach the assembly from its place."))
