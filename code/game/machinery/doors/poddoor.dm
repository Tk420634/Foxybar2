...
/obj/machinery/door/poddoor/attackby(obj/item/W, mob/user, params)
	...
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(density)
			to_chat(user, span_warning("I need to open [src] before opening its maintenance panel."))
			return
	...
	if(panel_open && density == FALSE)
		if(W.tool_behaviour == TOOL_MULTITOOL && deconstruction == BLASTDOOR_FINISHED)
			...
			if(change_id)
				...
				to_chat(user, span_notice("I change the ID to [id]."))

		else if(W.tool_behaviour == TOOL_CROWBAR && deconstruction == BLASTDOOR_FINISHED)
			to_chat(user, span_notice("I start to remove the airlock electronics."))
			...
			to_chat(user, span_notice("I start to remove the internal cables."))
			if(W.use_tool(src, user, 100, volume=50))
			...
			to_chat(user, span_notice("I start tearing apart the [src]."))
