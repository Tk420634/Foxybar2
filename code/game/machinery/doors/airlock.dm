...
/obj/machinery/door/airlock/proc/bolt()
	...
	audible_message(span_italic("I hear a click from the bottom of the door."), null,  1)
	update_icon()
		...
		if((HAS_TRAIT(H, TRAIT_DUMB)) && Adjacent(user))
			...
			if(!istype(H.head, /obj/item/clothing/head/helmet))
				H.visible_message(span_danger("[user] headbutts the airlock."), \
									span_userdanger("I headbutt the airlock!"))
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/screwdriver))
		...
		if(detonated)
			...
			return
		to_chat(user, span_warning("I apply [C]. Next time someone opens the door, it will explode."))
		panel_open = FALSE
		update_icon()
...
/obj/machinery/door/airlock/try_to_crowbar(obj/item/I, mob/living/user)
	...
	if(panel_open && charge)
		to_chat(user, span_notice("I carefully start removing [charge] from [src]..."))
		if(!I.use_tool(src, user, 150, volume=50))
			to_chat(user, span_warning("I slip and [charge] detonates!"))
			charge.ex_act(EXPLODE_DEVASTATE)
			user.DefaultCombatKnockdown(60)
			return
		user.visible_message(span_notice("[user] removes [charge] from [src]."), \
							span_notice("I gently pry out [charge] from [src] and unhook its wires."))
	...
	if(beingcrowbarred && panel_open && ((obj_flags & EMAGGED) || (density && welded && !operating && !hasPower() && !locked)))
		...
		if(!beingcrowbarred) //being fireaxe'd
			...
			if(!axe.wielded)
				to_chat(user, span_warning("I need to be wielding \the [axe] to do that!"))
				return
