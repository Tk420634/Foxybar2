...
/datum/component/virtual_reality/proc/quit(deathcheck = FALSE, cleanup = FALSE, mob/override)
	...
	if(!session_paused)
		...
		if(!dreamer) //This shouldn't happen.
			...
			to_chat(M, span_warning("I feel a dreadful sensation, something terrible happened. You try to wake up, but you find yourself unable to..."))
			qdel(src)
			return
		if(level_below?.parent)
			...
			if(deathcheck)
				to_chat(dreamer, span_warning("I feel everything fading away..."))
				dreamer.death(FALSE)
		mastermind.current.audiovisual_redirect = null
		if(!cleanup)
			if(level_above)
