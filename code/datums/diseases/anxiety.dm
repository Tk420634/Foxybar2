...
/datum/disease/anxiety/stage_act()
	...
	switch(stage)
		if(2) //also changes say, see say.dm
			if(prob(5))
				to_chat(affected_mob, span_notice("I feel anxious."))
		if(3)
			if(prob(10))
				to_chat(affected_mob, span_notice("My stomach flutters."))
			if(prob(5))
				to_chat(affected_mob, span_notice("I feel panicky."))
			if(prob(2))
				to_chat(affected_mob, span_danger("You're overtaken with panic!"))
				affected_mob.confused += (rand(2,3))
		if(4)
			if(prob(10))
				to_chat(affected_mob, span_danger("I feel butterflies in your stomach."))
			if(prob(5))
				affected_mob.visible_message(span_danger("[affected_mob] stumbles around in a panic."), \
												span_userdanger("I have a panic attack!"))
			...
			if(prob(2))
				affected_mob.visible_message(span_danger("[affected_mob] coughs up butterflies!"), \
													span_userdanger("I cough up butterflies!"))
