...
/datum/disease/beesease/stage_act()
	...
	switch(stage)
		if(2) //also changes say, see say.dm
			if(prob(2))
				to_chat(affected_mob, span_notice("I taste honey in your mouth."))
		if(3)
			if(prob(10))
		...
		if(4)
			if(prob(10))
				affected_mob.visible_message(span_danger("[affected_mob] buzzes."), \
												span_userdanger("My stomach buzzes violently!"))
			if(prob(5))
				to_chat(affected_mob, span_danger("I feel something moving in your throat."))
			if(prob(1))
				affected_mob.visible_message(span_danger("[affected_mob] coughs up a swarm of bees!"), \
													span_userdanger("I cough up a swarm of bees!"))
