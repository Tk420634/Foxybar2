		...
		if(1)
			if(restcure)
				if(affected_mob.lying && prob(30))
					...
					return
			if (prob(8))
				to_chat(affected_mob, span_danger("My head hurts."))
			if (prob(9))
				to_chat(affected_mob, "I feel a tingling sensation in your chest.")
			if (prob(9))
				to_chat(affected_mob, span_danger("I feel angry."))
		if(2)
			if(restcure)
				if(affected_mob.lying && prob(20))
					...
					return
			if (prob(8))
				to_chat(affected_mob, span_danger("My skin feels loose."))
			if (prob(10))
			...
			if (prob(4))
				to_chat(affected_mob, span_danger("My stomach churns."))
		if(3)
			if(restcure)
				if(affected_mob.lying && prob(20))
					to_chat(affected_mob, span_notice("I feel better."))
					...
					return
			if (prob(10))
				to_chat(affected_mob, span_danger("My entire body vibrates."))
