...
/datum/disease/transformation/mutant
	...
	stage3 = list()
	stage4 = list()
	stage5 = list(span_danger("I don't feel like yourself anymore!"))
...
/datum/disease/transformation/mutant/unstable/stage_act()
	...
	switch(stage)
		if(2)
			if (prob(8))
				to_chat(affected_mob, span_danger("I feel weird..."))
		if(3)
			if (prob(12))
				to_chat(affected_mob, span_danger("My skin twitches..."))
		...
		if(4)
			...
			if (prob(15))
				to_chat(affected_mob, span_danger("My skin begins to shift, hurting like hell!"))
				affected_mob.emote("scream")
				affected_mob.Jitter(4)
			if (prob(6))
				to_chat(affected_mob, span_danger("My body shuts down for a moment!"))
	...
	switch(stage)
		...
		if(4)
			if (prob(20))
				...
				affected_mob.emote("scream")
			if (prob(15))
				to_chat(affected_mob, span_warning("My skin begins to shift, it hurts, but only for a moment..?"))
				affected_mob.emote("cry")
			if (prob(5))
	...
	switch(stage)
		...
		if(2)
			...
			if(prob(2))
				...
				to_chat(affected_mob, span_warning("My chest hurts."))
			if(prob(4))
				to_chat(affected_mob, span_warning("I feel a cold sweat form."))
		if(3)
			if(prob(2))
			...
			if(prob(4))
				to_chat(affected_mob, span_danger("My head feels dizzy..."))
				affected_mob.adjustStaminaLoss(10)
		if(4) // That's the part where you start dying
			if(prob(2))
				to_chat(affected_mob, span_userdanger("I feel as if your organs just exploded!"))
			...
			if(prob(3))
				to_chat(affected_mob, span_warning("I should probably lie down for a moment..."))
		...
		if(5) // That's the part where you die for real
			...
			if(prob(5))
				to_chat(affected_mob, span_userdanger("I feel as if all your organs just exploded!"))
				affected_mob.emote("scream")
