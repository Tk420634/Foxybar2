...
/datum/disease/wizarditis/stage_act()
	...
	switch(stage)
		if(2)
			if(prob(1)&&prob(50))
				affected_mob.say(pick("I shall not pass!", "Expeliarmus!", "By Merlins beard!", "Feel the power of the Dark Side!"), forced = "wizarditis")
			if(prob(1)&&prob(50))
				to_chat(affected_mob, span_danger("I feel [pick("that you don't have enough mana", "that the winds of magic are gone", "an urge to summon familiar")]."))
		...
		if(3)
			...
			if(prob(1)&&prob(50))
				to_chat(affected_mob, span_danger("I feel [pick("the magic bubbling in your veins","that this location gives you a +1 to INT","an urge to summon familiar")]."))
		...
		if(4)
			...
			if(prob(1))
				...
				return
			if(prob(1)&&prob(50))
				to_chat(affected_mob, span_danger("I feel [pick("the tidal wave of raw power building inside","that this location gives you a +2 to INT and +1 to WIS","an urge to teleport")]."))
