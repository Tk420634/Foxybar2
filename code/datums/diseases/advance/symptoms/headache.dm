...
/datum/symptom/headache/Activate(datum/disease/advance/A)
	...
	if(power < 2)
		if(prob(base_message_chance) || A.stage >=4)
			to_chat(M, span_warning("[pick("My head hurts.", "My head pounds.")]"))
	if(power >= 2 && A.stage >= 4)
		to_chat(M, span_warning("[pick("My head hurts a lot.", "My head pounds incessantly.")]"))
		M.adjustStaminaLoss(25)
	if(power >= 3 && A.stage >= 5)
		to_chat(M, span_userdanger("[pick("My head hurts!", "I feel a burning knife inside your brain!", "A wave of pain fills your head!")]"))
		M.Stun(35)
