	...
	switch(A.stage)
		if(4, 5)
			...
			if(HAS_TRAIT_FROM(M, TRAIT_BLIND, EYE_DAMAGE))
				if(prob(20))
					to_chat(M, span_notice("My vision slowly returns..."))
					M.cure_blind(EYE_DAMAGE)
					M.cure_nearsighted(EYE_DAMAGE)
					M.blur_eyes(35)

				else if(HAS_TRAIT_FROM(M, TRAIT_NEARSIGHT, EYE_DAMAGE))
					to_chat(M, span_notice("I can finally focus your eyes on distant objects."))
					M.cure_nearsighted(EYE_DAMAGE)
					M.blur_eyes(10)
			...
			if(prob(base_message_chance))
				to_chat(M, span_notice("[pick("My eyes feel great.","I feel like your eyes can focus more clearly.", "I don't feel the need to blink.","My ears feel great.","My healing feels more acute.")]"))
