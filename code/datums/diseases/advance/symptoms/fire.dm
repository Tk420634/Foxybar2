	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance) && !suppress_warning)
				to_chat(M, span_warning("[pick("I feel hot.", "I hear a crackling noise.", "I smell smoke.")]"))
		if(4)
			Firestacks_stage_4(M, A)
			M.IgniteMob()
			to_chat(M, span_userdanger("My skin bursts into flames!"))
			M.emote("scream")
		if(5)
			Firestacks_stage_5(M, A)
			M.IgniteMob()
			to_chat(M, span_userdanger("My skin erupts into an inferno!"))
			M.emote("scream")
	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance))
				to_chat(M, span_warning("[pick("My veins boil.", "I feel hot.", "I smell meat cooking.")]"))
		if(4)
			Alkali_fire_stage_4(M, A)
			M.IgniteMob()
			to_chat(M, span_userdanger("My sweat bursts into flames!"))
			M.emote("scream")
		if(5)
