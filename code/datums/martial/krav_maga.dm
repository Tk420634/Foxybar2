...
/datum/martial_art/krav_maga/disarm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(CHECK_MOBILITY(D, MOBILITY_STAND))
		D.visible_message(span_danger("[A] reprimands [D]!"), \
					span_userdanger("You're slapped by [A]!"), span_hear("I hear a sickening sound of flesh hitting flesh!"), COMBAT_MESSAGE_RANGE, A)
		to_chat(A, span_danger("I jab [D]!"))
		A.do_attack_animation(D, ATTACK_EFFECT_PUNCH)
		...
		D.visible_message(span_danger("[A] reprimands [D]!"), \
					span_userdanger("You're manhandled by [A]!"), span_hear("I hear a sickening sound of flesh hitting flesh!"), COMBAT_MESSAGE_RANGE, A)
		to_chat(A, span_danger("I stomp [D]!"))
		A.do_attack_animation(D, ATTACK_EFFECT_KICK)
		playsound(D, 'sound/effects/hit_punch.ogg', 50, TRUE, -1)
		D.apply_damage(damage*2 + 20, STAMINA, affecting, armor_block)
		log_combat(A, D, "stomped nonlethally")
	if(damage >= stunthreshold)
		D.visible_message(span_warning("[D] sputters and recoils in pain!"), span_userdanger("I recoil in pain as you are jabbed in a nerve!"))
