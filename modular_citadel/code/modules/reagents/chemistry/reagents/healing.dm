...
/datum/reagent/fermi/yamerol/on_mob_life(mob/living/carbon/C)
	...
	if(T)
		if(T.name == "fluffy tongue")
			...
			qdel(T)
			nT.Insert(C)
			to_chat(C, span_notice("I feel your tongue.... unfluffify...?"))
...
/datum/reagent/fermi/yamerol/overdose_process(mob/living/carbon/C)
	...
	if(current_cycle == 1)
		to_chat(C, span_notice("I feel the Yamerol sooth your tongue and lungs."))
	if(current_cycle > 10)
		if(!C.getorganslot(ORGAN_SLOT_TONGUE))
			...
			if(C.dna && C.dna.species && C.dna.species.mutanttongue)
				...
				T = new()
			T.Insert(C)
			to_chat(C, span_notice("I feel your tongue reform in your mouth."))
			holder.remove_reagent(type, 10)
		else
		...
		if(!C.getorganslot(ORGAN_SLOT_LUNGS))
			...
			L.Insert(C)
			to_chat(C, span_notice("I feel the yamerol merge in your chest."))
			holder.remove_reagent(type, 10)
	C.adjustOxyLoss(-3)
	..()
		...
		if(method == PATCH)	//Needs to actually be applied via patch / hypo / medspray and not just beakersplashed.
			if (C.stat == DEAD)
				...
				if(active_tissue && active_tissue.borrowed_health)
					...
					imperfect = TRUE
				to_chat(M, span_danger("I feel your flesh [imperfect ? "partially and painfully" : ""] merge with the synthetic tissue! It stings like hell[imperfect ? " and is making you feel terribly sick" : ""]!"))
		...
		if(data["grown_volume"] > 175) //I don't think this is even possible, but damn to I want to see if someone can (bare in mind it takes 2s to grow 0.05u)
			if(volume >= 14)
				if(C.regenerate_organs(only_one = TRUE))
					...
					to_chat(C, span_notice("I feel something reform inside of you!"))
