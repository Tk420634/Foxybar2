...
/obj/item/dogborg/sleeper/compactor/afterattack(atom/movable/target, mob/living/silicon/user, proximity)//GARBO NOMS
	...
	if(length(contents) > (max_item_count - 1))
		to_chat(user,span_warning("My [src] is full. Eject or process contents to continue."))
		return
	if(isitem(target))
		...
		if(I.w_class > WEIGHT_CLASS_NORMAL)
			...
			return
		user.visible_message(span_warning("[hound.name] is ingesting [I] into their [src.name]."), span_notice("I start ingesting [target] into your [src.name]..."))
		if(do_after(user, 1.5 SECONDS, target) && length(contents) < max_item_count)
			I.forceMove(src)
			I.visible_message(span_warning("[hound.name]'s garbage processor groans lightly as [I] slips inside."), span_notice("My garbage compactor groans lightly as [I] slips inside."))
	...
	if(iscarbon(target) || issilicon(target))
		...
		if(!CHECK_BITFIELD(trashman.vore_flags,DEVOURABLE))
			...
			return
		if(patient)
			to_chat(user,span_warning("My [src] is already occupied."))
			return
		if(trashman.buckled)
			to_chat(user,span_warning("[trashman] is buckled and can not be put into your [src]."))
			return
		user.visible_message(span_warning("[hound.name] is ingesting [trashman] into their [src]."), span_notice("I start ingesting [trashman] into your [src.name]..."))
		if(do_after(user, 3 SECONDS, trashman) && !patient && !trashman.buckled && length(contents) < max_item_count)
			...
			update_gut(user)
			user.visible_message(span_warning("[hound.name]'s garbage processor groans lightly as [trashman] slips inside."), span_notice("My garbage compactor groans lightly as [trashman] slips inside."))
